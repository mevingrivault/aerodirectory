...
import type { PokedexStats } from "@aerodirectory/shared";
...
export default function PokedexPage() {
  ...
  const { data: statsRes } = useQuery({
    queryKey: ["pokedex-stats"],
    queryFn: () => apiClient.get<PokedexStats>("/visits/stats"),
    enabled: !!user,
  });
  ...
  if (!user) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <h1 className="text-2xl font-bold mb-2">Pilot Logbook</h1>
        <p className="text-muted-foreground">
          <Link href="/login" className="text-primary hover:underline">
            Sign in
          </Link>{" "}
          to track your visits and earn badges.
        </p>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <div className="text-sm text-muted-foreground">Est. Distance</div>
              </CardContent>
            </Card>
          </div>
      ...
      <Card>
        ...
        <CardContent>
          {visits.length === 0 ? (
            <p className="text-muted-foreground">
              No visits yet. Start exploring aerodromes!
            </p>
          ) : (
